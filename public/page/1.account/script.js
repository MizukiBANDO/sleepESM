...
document.getElementById('registerForm').addEventListener('submit', function(e) {
  ...
  if (!name) {
    document.getElementById('registerResult').textContent = '名前を入力してください。';
    return;
  }

  localStorage.setItem('userName', name);
  document.getElementById('registerResult').textContent = `「${name}」として登録されました。`;
});

document.getElementById("deleteRegister").addEventListener("click", function () {
  if (localStorage.getItem("userName")) {
      const confirmed = confirm("登録された名前を削除しますか？");
      if (confirmed) {
          localStorage.removeItem("userName");
          document.getElementById("registerResult").textContent = "登録情報を削除しました。";
      }
  } else {
      document.getElementById("registerResult").textContent = "登録された名前はありません。";
  }
});

const deleteBtn = document.getElementById("deleteRegister");
if (!localStorage.getItem("userName")) {
...
if (storedName) {
  document.getElementById('userNameDisplay').textContent = `ログイン中: ${storedName}`;
  document.getElementById('loggedInUserInfo').classList.remove('d-none');
  document.getElementById('registerPrompt')?.classList.add('d-none');

  // 除外チェックボックスの状態を読み込み・設定
  const checkboxes = document.querySelectorAll('[data-exclude-key]');
  const excluded = JSON.parse(localStorage.getItem("excludedItems") || "[]");

  checkboxes.forEach(checkbox => {
    const key = checkbox.dataset.excludeKey;
    checkbox.checked = excluded.includes(key);

    checkbox.addEventListener("change", () => {
      const updated = new Set(JSON.parse(localStorage.getItem("excludedItems") || "[]"));
      if (checkbox.checked) {
        updated.add(key);
      } else {
        updated.delete(key);
      }
      localStorage.setItem("excludedItems", JSON.stringify([...updated]));
    });
  });
}
